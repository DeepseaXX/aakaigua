...
这篇文章介绍的方法，相比使用 DLSS Mod 也就是是省事了一点，少了一点兼容性问题例如不能与卫月交互。但其实我已经很久没遇到过无法交互卫月的问题了，所以请自己判断选择。
...
首先只要是 RTX 显卡都可以用 DLSS 最新模型，例如 30 系只是不能使用 DLSS4 中的倍帧而已。
...
3. 搜索目标字段在文件中搜索关键词 `final fantasy`，或 `ffxiv` 。（此时你可以看到应该定位到了附近有 `"DisplayName": "Final Fantasy XIV - DX9"`,                `"ShortName": "final_fantasy_xiv_kr"`，还有路径中有 `上海 XX 科技有限公司`字样）~~（别问为什么是 DX9，为什么是 kr，我也不知道）~~
...
（我在重启之后有几个项被重置成了 true，但是对修改模型没影响）5. 保存并关闭文件。随后重启电脑。
